...
class _TierListSchemeDetailScreenState
                ...
                child: Column(
                  children: [
                    _TierListEditorToolbar(
                      ...
                      onSave: save,
                      onExport: () =>
                          _exportBoardImage(displayScheme, heroesById),
                      onToggleLaneBoard: () {
                        setState(() {
                      ...
                      onToggleHeroPool: () =>
                          setState(() => _isHeroPoolOpen = !_isHeroPoolOpen),
                    ),
                    Expanded(
                      child: Padding(
                        ...
                        child: _TierListEditorWorkspace(
                          ...
                          onHeroReturnedToPool: _removeHeroFromAllRows,
          ...
          children: [
            RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  AppSectionHeader(
                    title: AppLocalizations.of(context).translate('tierDetail'),
                  ),
                  const SizedBox(height: 8),
                  Text(
                    AppLocalizations.of(
                      context,
                    ).toolSubtitle('/tools/tier-list'),
            ...
            if (widget.initialEditMode)
              Positioned(
                ...
                child: FloatingActionButton.extended(
                  ...
                  label: Text(
                    AppLocalizations.of(context).translate('tierSaveChanges'),
                  ),
                ),
              ),
          ],
    ...
    setState(() {
      _editedRows = [
        for (final row in _editedRows)
          if (row.id == rowId)
            row.copyWith(
              heroIds: [...row.heroIds.where((id) => id != heroId), heroId],
              heroCount: row.heroIds.where((id) => id != heroId).length + 1,
            )
          else
            row.copyWith(
              heroIds: row.heroIds.where((id) => id != heroId).toList(),
  ...
  void _removeHeroFromAllRows(int heroId) {
    setState(() {
      _editedRows = [
        for (final row in _editedRows)
          row.copyWith(
  ...
  Future<void> _exportBoardImage(
    TierListSchemeSummary scheme,
    Map<int, HeroSummary> heroesById,
  ) async {
    final exportKey = GlobalKey();
    OverlayEntry? exportEntry;
    try {
      final rowHeight = _showLaneBoard ? 112.0 : 104.0;
      final exportHeight =
          50 +
          (_showLaneBoard ? 34 : 0) +
          scheme.rows.length * rowHeight +
          math.max(0, scheme.rows.length - 1) * 10;
      exportEntry = OverlayEntry(
            ...
            child: Stack(
              ...
              children: [
                OverflowBox(
                  ...
                  minWidth: 1200,
                  maxWidth: 1200,
                  minHeight: exportHeight,
                  maxHeight: exportHeight,
                  child: RepaintBoundary(
                    key: exportKey,
                    child: _TierExportBoard(
                      scheme: scheme,
                      heroesById: heroesById,
                      showLaneGroups: _showLaneBoard,
                      rowHeight: rowHeight,
                    ),
                  ),
                ),
      ...
      Overlay.of(context, rootOverlay: true).insert(exportEntry);
      await WidgetsBinding.instance.endOfFrame;
      await Future<void>.delayed(const Duration(milliseconds: 200));
      final boundary =
          exportKey.currentContext?.findRenderObject()
              as RenderRepaintBoundary?;
      if (boundary == null) throw StateError('Tier export is not ready');
      ...
      debugPrint('Failed to export tier list: $error');
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              _ToolbarButton(
                buttonKey: const ValueKey('tier-hero-pool-toggle'),
                tooltip: isHeroPoolOpen ? 'Close hero pool' : 'Open hero pool',
                icon: isHeroPoolOpen
              ...
              _ToolbarButton(
                ...
                tooltip: AppLocalizations.of(context).translate('tierLaneView'),
              ...
              _ToolbarButton(
                ...
                tooltip: AppLocalizations.of(context).translate('tierExport'),
...
class _TierListEditorWorkspace extends StatelessWidget {
  const _TierListEditorWorkspace({
    ...
    required this.onHeroReturnedToPool,
  ...
  final ValueChanged<int> onHeroReturnedToPool;
        ...
        return Row(
          ...
          children: [
            ...
            AnimatedContainer(
              ...
              width: isHeroPoolOpen ? 8 : 0,
            ),
            AnimatedContainer(
              duration: const Duration(milliseconds: 280),
              curve: Curves.easeOutCubic,
              width: isHeroPoolOpen ? poolWidth : 0,
              child: ClipRect(
                child: Align(
                  alignment: Alignment.centerRight,
                  widthFactor: isHeroPoolOpen ? 1 : 0,
                  child: SizedBox(
                    width: poolWidth,
                    child: _HeroPoolPanel(
                      ...
                      assignedHeroIds: scheme.rows
                          ...
                          .toSet(),
                      onHeroReturned: onHeroReturnedToPool,
                    ),
                  ),
                ),
              ),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ReorderableDelayedDragStartListener(
                      ...
                      child: Padding(
                        ...
                        // 行高随内容自适应：英雄放满一行后自动换行。
                        child: _TierRowDetail(
                          row: row,
                          heroesById: heroesById,
                          isEditMode: true,
                          exposeDropKey: true,
                          showLaneGroups: showLaneBoard && lanePosition == null,
...
class _HeroPoolPanelState extends State<_HeroPoolPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    return DragTarget<int>(
      onAcceptWithDetails: (details) => widget.onHeroReturned(details.data),
          ...
          children: [
            ...
            if (isSaving)
              ...
              IconButton.filledTonal(
                key: const ValueKey('tier-list-save-changes-top'),
                tooltip: l10n.translate('tierSaveChanges'),
                onPressed: onSave,
            ...
            Row(
              children: [
                IconButton(
                  ...
                  tooltip: l10n.translate('commonMoveUp'),
                  onPressed: canMoveUp ? onMoveUp : null,
                  icon: const Icon(Icons.keyboard_arrow_up_rounded),
                ),
                IconButton(
                  key: ValueKey('tier-row-move-down-${row.id}'),
                  tooltip: l10n.translate('commonMoveDown'),
                ...
                Expanded(
                  child: TextFormField(
                    ...
                    decoration: InputDecoration(
                      labelText: index == 0
                          ? l10n.translate('statsTier')
                          : '${l10n.translate('statsTier')} ${index + 1}',
          ...
          children: [
            Expanded(
              child: TextField(
                ...
                decoration: InputDecoration(
                  labelText:
                      '${l10n.translate('commonAdd')} ${l10n.translate('statsHero')} ID',
            ...
            IconButton.filledTonal(
              ...
              tooltip:
                  '${l10n.translate('commonAdd')} ${l10n.translate('statsHero')}',
              onPressed: () {
  ...
  Widget build(BuildContext context) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                if (!isEditMode) ...[
                  Text(
                    row.heroCount == 1 ? '1 hero' : '${row.heroCount} heroes',
                    style: Theme.of(context).textTheme.titleSmall?.copyWith(
                      color: context.hokTheme.onSurfaceStrong,
                      fontWeight: FontWeight.w800,
                    ),
    ...
    final content = DecoratedBox(
      ...
      child: isEditMode
          ...
          : SizedBox(height: 92, child: rowBody),
    );
...
class _TierRowColorMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return IconButton(
      ...
      tooltip: l10n.translate('statsTier'),
      padding: EdgeInsets.zero,
...
class _TierHeroToken extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return LongPressDraggable<int>(
      ...
      childWhenDragging: Opacity(opacity: 0.22, child: token),
      child: token,
    );
  }
}

class _TierExportBoard extends StatelessWidget {
  const _TierExportBoard({
    required this.scheme,
    required this.heroesById,
    required this.showLaneGroups,
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: Padding(
        ...
        child: DecoratedBox(
          decoration: BoxDecoration(
            color: const Color(0xFF050B19),
            borderRadius: BorderRadius.circular(18),
            border: Border.all(color: const Color(0xFF24324B), width: 2),
          ),
          child: Padding(
            padding: const EdgeInsets.all(10),
            child: Column(
              children: [
                ...
                for (var index = 0; index < scheme.rows.length; index++) ...[
                  SizedBox(
                    height: rowHeight,
                    child: _TierRowDetail(
                      row: scheme.rows[index],
                      heroesById: heroesById,
                      isEditMode: false,
                      exposeDropKey: false,
                      showLaneGroups: showLaneGroups,
                      onHeroAdded: (_) {},
                    ),
                  ),
                  if (index != scheme.rows.length - 1)
                    const SizedBox(height: 10),
                ],
              ],
            ),
